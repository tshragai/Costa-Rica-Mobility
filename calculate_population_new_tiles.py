#!/usr/bin/env python3
"""
Calculate population for the new costa_rica_activity_tiles.gpkg using both WorldPop and GHSL.
"""
...
def main():
    """Calculate population for the new activity tiles."""
    print("=== Population Calculation for New Activity Tiles ===")
    
    # Initialize GEE
    ee.Initialize()
    print("✓ Google Earth Engine initialized successfully")
    
    # Path to the new GPKG file
    gpkg_path = "Data/costa_rica_activity_tiles.gpkg"
    ...
    try:
        # Read the GPKG file
        print("Loading activity tiles from GPKG...")
        gdf = gpd.read_file(gpkg_path)
        print(f"✓ Loaded {len(gdf)} tiles from GPKG")
        
        # Convert to GeoJSON for GEE
        print("Converting to GeoJSON for GEE...")
        geojson_data = json.loads(gdf.to_json())
        
        # Create FeatureCollection from GeoJSON
        tiles = ee.FeatureCollection(geojson_data)
        print("✓ FeatureCollection created for GEE")
        ...
        if 'tile_id' not in df_combined.columns:
            df_combined['tile_id'] = range(len(df_combined))
        
        # Reorder columns for better readability
        df_combined = df_combined[['tile_id', 'worldpop_population', 'ghsl_population_2025', 'geometry']]
        ...
        # Save GeoJSON
        output_geojson = "Data/costa_rica_activity_tiles_population.geojson"
        ...
        output_csv = "Data/costa_rica_activity_tiles_population.csv"
        ...
        print(f"\n🎉 Success! Population data ready for sf import in R!")
        ...
        print(f"\n📖 To read in R:")
        print(f"   library(sf)")
        print(f"   tiles_pop <- st_read('Data/costa_rica_activity_tiles_population.geojson')")
