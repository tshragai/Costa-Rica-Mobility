...
## --- Read in activity data from Google Drive ---
# Download from Google Drive and read
drive_auth()
target_folder <- as_id("1j9yE_C6okSNO73RROigLONOU9VGZ9wJa")
tmp_csv <- tempfile(fileext = ".csv")

# List files in the folder and find the one we need
folder_files <- drive_ls(path = target_folder)
file_info <- folder_files[folder_files$name == "all_processed_combined.csv", ]

if (nrow(file_info) > 0) {
  drive_download(file = as_id(file_info$id[1]), path = tmp_csv, overwrite = TRUE)
  ca_data <- read.csv(tmp_csv)
  unlink(tmp_csv)  # Clean up temporary file
  message("✓ Successfully loaded all_processed_combined.csv from Google Drive")
} else {
  stop("File 'all_processed_combined.csv' not found in Google Drive folder. Available files: ", 
...
# Write to temporary file first, then upload to Google Drive
tmp_gpkg <- tempfile(fileext = ".gpkg")
st_write(squares_ll, tmp_gpkg, delete_dsn = TRUE)

# Upload to Google Drive
message("Uploading costa_rica_activity_tiles.gpkg to Google Drive...")
drive_auth()
target_folder <- as_id("1j9yE_C6okSNO73RROigLONOU9VGZ9wJa")
drive_upload(
  tmp_gpkg,
  name = "costa_rica_activity_tiles.gpkg",
  path = target_folder,
  overwrite = TRUE
)
...
```{r population data}
# Read population data from Data folder
data_dir <- path.expand("~/Costa-Rica-Mobility/Data")
...
# Write all files to temporary locations first, then upload to Google Drive
target_folder <- as_id("1j9yE_C6okSNO73RROigLONOU9VGZ9wJa")
drive_auth()

# Save GPKG files (projected coordinates)
tmp_overall_gpkg <- tempfile(fileext = ".gpkg")
tmp_by_ds_gpkg <- tempfile(fileext = ".gpkg")
st_write(tile_activity_overall_sf, tmp_overall_gpkg, delete_dsn = TRUE)
st_write(tile_activity_by_ds_sf, tmp_by_ds_gpkg, delete_dsn = TRUE)
...
# Save GeoJSON files (WGS84)
tmp_overall_geojson <- tempfile(fileext = ".geojson")
tmp_by_ds_geojson <- tempfile(fileext = ".geojson")
st_write(tile_activity_overall_ll, tmp_overall_geojson, delete_dsn = TRUE)
st_write(tile_activity_by_ds_ll, tmp_by_ds_geojson, delete_dsn = TRUE)

# Save CSV file
tmp_ca_aug <- tempfile(fileext = ".csv")
fwrite(ca_aug, tmp_ca_aug)

# Upload all files to Google Drive
message("Uploading files to Google Drive...")
drive_upload(tmp_overall_gpkg, name = "activity_by_pop_tiles_overall.gpkg", path = target_folder, overwrite = TRUE)
drive_upload(tmp_by_ds_gpkg, name = "activity_by_pop_tiles_by_ds.gpkg", path = target_folder, overwrite = TRUE)
drive_upload(tmp_overall_geojson, name = "activity_by_pop_tiles_overall_wgs84.geojson", path = target_folder, overwrite = TRUE)
drive_upload(tmp_by_ds_geojson, name = "activity_by_pop_tiles_by_ds_wgs84.geojson", path = target_folder, overwrite = TRUE)
drive_upload(tmp_ca_aug, name = "ca_aug.csv", path = target_folder, overwrite = TRUE)
message("✓ Successfully uploaded all files to Google Drive folder")

# Clean up temporary files
unlink(c(tmp_overall_gpkg, tmp_by_ds_gpkg, tmp_overall_geojson, tmp_by_ds_geojson, tmp_ca_aug))
...
# List files in the folder and find the one we need
folder_files <- drive_ls(path = target_folder)
file_info <- folder_files[folder_files$name == "activity_by_pop_tiles_overall_wgs84.geojson", ]

if (nrow(file_info) > 0) {
  drive_download(file = as_id(file_info$id[1]), path = tmp_geojson, overwrite = TRUE)
  tile_activity_overall_ll <- st_read(tmp_geojson)
  unlink(tmp_geojson)  # Clean up temporary file
  message("✓ Successfully loaded activity_by_pop_tiles_overall_wgs84.geojson from Google Drive")
} else {
  stop("File 'activity_by_pop_tiles_overall_wgs84.geojson' not found in Google Drive folder. Available files: ", 
       paste(folder_files$name, collapse = ", "))
}  
...
st_write(districts, "data/Costa_Rica_districts.gpkg", delete_dsn = TRUE)
