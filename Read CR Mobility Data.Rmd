...
# --- 0) (Optional) pick a subset of ds if there are many ---
# e.g., the 6 most recent ds values
# tile_activity_by_ds_sf <- tile_activity_by_ds_sf %>%
#   mutate(ds = as.Date(ds)) %>%
#   arrange(ds) %>%
#   group_by(tile_id) %>% ungroup() %>%
#   filter(ds %in% tail(sort(unique(ds)), 6))

# --- 1) Choose mapping variable: raw or log1p ---
df_map <- tile_activity_by_ds_sf %>%
  mutate(activity_log1p = log1p(activity))

# Use raw:
#value_col <- "activity"
# Or use log1p (uncomment the next line and comment the one above):
 value_col <- "activity_log1p"

# --- 2) Global limits so colors are comparable across facets ---
vrng <- range(df_map[[value_col]], na.rm = TRUE)

# --- 3) Nice labels for legend ---
leg_title <- if (value_col == "activity") "Activity (pop-weighted)" else "log(1 + activity)"

# --- 4) Faceted map ---
p <- ggplot(df_map) +
  geom_sf(aes(fill = .data[[value_col]]), color = NA) +
  facet_wrap(~ ds) +
  scale_fill_viridis_c(name = leg_title, limits = vrng, na.value = "transparent") +
...
# ggsave("fig_activity_faceted_by_ds.png", p, width = 12, height = 8, dpi = 300)
