...
# District polygons (minimal version)
districts_min <- readRDS(file.path(data_dir, "districts_min.rds"))

# Population per tile data
pop_tiles <- st_read(file.path(data_dir, "costa_rica_activity_tiles_population.geojson"))

# Activity tiles with elevation data (for joining elevation to pop_tiles)
mob <- st_read(file.path(data_dir, "activity_by_pop_tiles_overall_wgs84.geojson")) %>%
  st_make_valid() %>%
  st_transform(4326)

# Malaria case points
mal23_pts <- readRDS(file.path(data_dir, "mal23_pts.rds"))

# Flow data with elevation
flows_all_elev <- readRDS(file.path(data_dir, "flows_all_elev.rds"))
